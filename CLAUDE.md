...
- **Content-Security-Policy** is set in `SecurityConfig.CSP`. `script-src` still carries `'unsafe-inline'` because the pages depend on ~343 inline `onclick=`-style handlers (~191 in HTML, ~152 string-built by the JS modules); a nonce would silently disable every one of them, so removing the handlers is a prerequisite, not an afterthought. Everything else is locked down — notably `object-src 'none'`, `base-uri 'self'`, `form-action 'self'`, and allowlisted `connect-src`/`img-src`, which is what denies an injected script anywhere to exfiltrate to. No `'unsafe-eval'`: nothing in this codebase evals, and shpjs/turf/MapLibre were each verified to run without it.
