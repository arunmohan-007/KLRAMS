...
**The geoid model is entered by hand** (`drone_dataset.geoid_model`), not parsed. Pix4D and Metashape write the horizontal CRS into the GeoTIFF and leave the vertical side to their PDF report, so the file usually cannot answer it. Kept in its own column rather than merged into `geo_details`, which stays strictly what the FILE declares — the info panel labels it "recorded from documentation" so the two are never mistaken for equal authority. Settable at upload (DEM and contour types only) and editable afterwards via `PUT /api/drone/datasets/{id}/geoid`, because the report routinely arrives after the raster.
...
- Sign-in is rate-limited on **two** counters in `LoginAttemptService` (+ `LoginAttemptFilter`): 5 failures per client IP → 15-minute lockout, and 15 failures per **username** → 5-minute lockout. The second exists because the IP counter alone missed distributed guessing — spread across enough IPs, no single address ever reaches 5. The username threshold is looser and its lock shorter on purpose, because a per-account lock is something an insider can trigger against a colleague deliberately. Keys are namespaced `ip:` / `user:` and usernames are lower-cased, so alternating case cannot reset the counter.
- **The IP counter is only as good as the client IP.** `LoginAuditService.clientIp()` reads `X-Forwarded-For` by counting **`app.security.proxy-hops` entries in from the RIGHT** — never from the left, which the client forges freely. The hop count must match the real chain: 1 for nginx alone, 2 with Cloudflare/an edge in front. Set it too low and every visitor resolves to the inner proxy's own address, so five bad passwords by one person lock **the entire user base** out for 15 minutes (this happened in production). As a backstop, `LoginAttemptService` drops the IP counter entirely whenever the resolved address is private/loopback — a misconfigured hop count then degrades to username-only throttling instead of taking sign-in down. To settle the value for a given deployment, sign in as SUPER_ADMIN and GET **`/api/reports/connection`** (`ConnectionDiagnosticsController`): it echoes that request's own proxy headers hop by hop, each with the `proxyHopsToSelect` value that would pick it, plus a plain-language verdict.
- Password rules live in `PasswordPolicy.validate()`, called from all three places a password can be set (create / admin reset / self-service change). Length (8) is only the floor; it also refuses common passwords, repeated characters, keyboard and digit runs, the username itself, and site words like `kerala` / `klrams` / `pwd` — including when decorated with digits, so `Kerala@2024` is rejected. Deliberately **not** a "one upper, one digit, one symbol" rule (that produces `Password@1`, which this rejects). Note existing weak passwords are unaffected until next changed.
- **Content-Security-Policy** is set in `SecurityConfig.CSP`, and `script-src` is now `'self' https://unpkg.com` — **no `'unsafe-inline'`, no `'unsafe-eval'`**. An injected inline `<script>` or `onerror=` does not execute. `object-src 'none'`, `base-uri 'self'`, `form-action 'self'` and allowlisted `connect-src`/`img-src` mean injected script also has nowhere to exfiltrate to. `style-src` deliberately keeps `'unsafe-inline'` (inline `style=""` is everywhere and CSS is not an execution vector the way script is).
  **Adding a new external host (a basemap, a glyph server, a CDN) means adding it to `CSP` or it is silently blocked at runtime.** Tile/basemap hosts must go in BOTH `img-src` and `connect-src` — Leaflet loads tiles as `<img>`, MapLibre fetches them.
- **There are no inline `<script>` blocks and no inline `on*=` handlers anywhere**, and it must stay that way or the CSP above breaks the page. Every page's script lives in its own `js/*-page.js` (or `js/map-*.js`); every control dispatches through `js/00-actions.js`:
  ```html
  <button data-act="save">                      <button data-act="del" data-args='["KPWD/SH/1"]'>
  <select data-change="klValue" data-args="setThing">   <input data-enter="search">
  ```
  `data-args` is a JSON array (a bare non-JSON string is passed as a single string argument). Dotted paths work and keep their owner as `this` (`data-act="SM.close"`); bare names get the element as `this`. Built-ins: `klValue`/`klChecked` forward a control's value, `klStop` stops propagation, `klHide`/`klRemoveClass` cover the old one-line expressions, `data-hide-on-error` replaces `onerror` on images, and `a[href="#"][data-act]` gets `preventDefault` automatically. `KLAct.args(...)` builds an escaped `data-args` attribute — use it instead of hand-escaping a value into markup.
