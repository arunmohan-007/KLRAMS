...
> CSV datasets use `ImportTemplateController`.
...
## CSV datasets with a template — the column-mapping wizard
...
Geometry imports (road network, full road network, district/constituency boundaries) have no
template mapping UI — `RoadUploadController` matches properties 1:1 against DB columns and
`BoundaryController` stores GeoJSON as-is.
...
- **No reprojection** — everything is EPSG:4326.
...
- **No mapping wizard for geometry imports** — there is no per-column target list to map
  shapefile/GeoJSON properties against.
